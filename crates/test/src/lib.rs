//! The `gecko_test` crate helps you to write Rust tests for your _programs_ with gecko.
//! See the macro [gecko_test] for more info.
//!
...
pub fn gecko_test(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let input_fn: ItemFn =
        syn::parse(input).expect("'gecko_test' attribute is applicable only to async fn");
    ...
    quote::quote_spanned!(input_fn_span=>
        ...
        // `#[gecko_client::tokio::test...` to make macros like `#[rstest]` work -
        ...
        async fn #input_fn_name(#input_fn_inputs) -> gecko_client::anyhow::Result<()> {
            let mut tester = gecko_client::Tester::with_root(#root);
