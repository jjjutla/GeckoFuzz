use crate::{
    ...
    config::{Config, CARGO_TOML, gecko_TOML},
};
...
pub(crate) const TESTS_WORKSPACE: &str = "gecko-tests";
...
impl TestGenerator {
    ...
    /// `gecko-tests/tests` directory with all the necessary files. Adds the
    /// `test.rs` file and generates `Cargo.toml` with `dependencies`. Updates root's `Cargo.toml`
    /// workspace members.
    ///
    /// The crate is generated from `gecko-tests` template located in `client/src/templates`.
    ///
    /// Before you start writing gecko tests do not forget to add your program as a dependency
    /// to the `gecko-tests/Cargo.toml`. For example:
    ///
    /// ```toml
    /// # <project_root>/gecko-tests/Cargo.toml
    /// # ...
    /// [dependencies]
    ...
    /// Creates the `gecko-tests` workspace with `tests` directory and empty `test.rs` file
    /// finally it generates the `Cargo.toml` file. Crate is generated from `gecko-tests`
    ...
    async fn generate_test_files(&self, root: &Path) {
        ...
        let test_content = include_str!(concat!(
            ...
            "/src/templates/gecko-tests/test.rs"
        ));
        ...
        let cargo_toml_content = include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/src/templates/gecko-tests/Cargo.toml.tmpl"
        ));
        self.create_file(&cargo_toml_path, CARGO_TOML, cargo_toml_content)
            .await?;
        self.add_program_deps(root, &cargo_toml_path).await?;

        let gecko_toml_path = root.join(gecko_TOML);
        let gecko_toml_content = include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/src/templates/gecko.toml.tmpl"
        ));
        self.create_file(&gecko_toml_path, gecko_TOML, gecko_toml_content)
            .await?;
    }

    /// Creates the `gecko-tests` workspace with `src/bin` directory and empty `fuzz_target.rs` file
    #[throws]
    async fn generate_fuzz_test_files(&self, root: &Path) {
        ...
        let fuzz_test_content = include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/src/templates/gecko-tests/fuzz_target.rs"
        ))
        .to_string();
        ...
        let workspace_path = root.join(TESTS_WORKSPACE);
        let cargo_toml_path = workspace_path.join(CARGO_TOML);
        self.add_feature_to_dep(root, &cargo_toml_path, "gecko-client", "fuzzing")
            .await?;
    }
    ...
    /// Adds `gecko-tests` workspace to the `root`'s `Cargo.toml` workspace members if needed.
    #[throws]
