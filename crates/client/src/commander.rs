...
pub enum Error {
    ...
    FuzzingFailed,
    #[error("gecko it not correctly initialized! The gecko-tests folder in the root of your project does not exist")]
    NotInitialized,
...
impl Commander {
    ...
    ///
    /// It's used internally by the [`#[gecko_test]`](gecko_test::gecko_test) macro.
    #[throws]
    ...
    pub async fn generate_program_client_deps(&self) {
        let gecko_dep = r#"gecko-client = "0.5.0""#.parse().unwrap();
        // @TODO replace the line above with the specific version or commit hash
        // when gecko is released or when its repo is published.
        // Or use both variants - path for gecko repo/dev and version/commit for users.
        // Some related snippets:
        //
        // println!("gecko Version: {}", std::env!("VERGEN_BUILD_SEMVER"));
        // println!("gecko Commit: {}", std::env!("VERGEN_GIT_SHA"));
        // https://docs.rs/vergen/latest/vergen/#environment-variables
        //
        // `gecko = "0.1.0"`
        // `gecko = { git = "https://github.com/Ackee-Blockchain/gecko.git", rev = "cf867aea87e67d7be029982baa39767f426e404d" }`
        ...
        for dep in iter::once(gecko_dep).chain(program_deps) {
    ...
    pub async fn parse_program_client_imports(&self) -> Vec<syn::ItemUse> {
        ...
        if use_modules.is_empty() {
            use_modules.push(syn::parse_quote! { use gecko_client::*; })
        }
        use_modules
    }
}
