...
pub const gecko_TOML: &str = "gecko.toml";
...
impl Config {
    pub fn new() -> Self {
        ...
        let s = fs::read_to_string(root.join(gecko_TOML).as_path())
            .expect("failed to read the gecko config file");
        let _config: _Config =
            toml::from_str(&s).expect("failed to parse the gecko config file");
        _config.into()
    }
